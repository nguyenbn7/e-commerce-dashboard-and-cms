...
type Response = InferResponseType<
	(typeof client.api.stores)[':storeId']['colors'][':id']['$delete']
>;
type Request = InferRequestType<(typeof client.api.stores)[':storeId']['colors'][':id']['$delete']>;
...
export function deleteColor(options: Options = {}) {
	const { onSuccess, onError } = options;

	const mutation = createMutation<Response, Error, Request>({
		mutationFn: async ({ param }) => {
			const response = await client.api.stores[':storeId']['colors'][':id']['$delete']({
				param
			});
